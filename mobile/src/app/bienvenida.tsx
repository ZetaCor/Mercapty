...
// mueven un poco más lento que el texto (efecto de profundidad).
...
const Hl = ({ children }: { children: ReactNode }) => (
  <T w={800} size={29} tight color={C.brand}>{children}</T>
);
...
export default function Bienvenida() {
  ...
  const pages = [
    {
      eyebrow: 'Canasta básica · Panamá',
      title: n > 1 ? <>El <Hl>precio más bajo</Hl>, en {n} supermercados a la vez</> : <>El <Hl>precio más bajo</Hl> de tu canasta básica</>,
      body: meta
        ? `Compara ${count(meta.products)} productos del súper y mira, tienda por tienda, dónde te sale más barato.`
    ...
    {
      eyebrow: 'Mi lista',
      title: <>Arma tu canasta y <Hl>ahorra en cada compra</Hl></>,
      body: 'Agrega lo que compras cada semana y te decimos si conviene comprar todo en un súper o repartir la compra entre varios.',
      art: <BasketArt />,
      size: [290, 330],
    },
    {
      eyebrow: 'Directo a tu súper',
      title: <>Compra donde está <Hl>más barato</Hl></>,
      body: 'Con un toque te llevamos a la tienda en línea con el mejor precio. La compra, el pago y la entrega los haces en tu súper.',
    ...
    <View style={styles.screen}>
      ...
      <View style={[styles.top, { paddingTop: insets.top + 10 }]}>
        ...
        <Pressable
          ...
          <T w={600} color={C.muted}>Saltar</T>
        </Pressable>
      </View>
      ...
      <Animated.ScrollView
        ...
        {pages.map((page, i) => {
          ...
          return (
            <Page key={page.eyebrow} i={i} x={x} width={width} height={pageH} artArea={artArea}
              art={<View style={{ transform: [{ scale }] }}>{page.art}</View>}>
              <View style={styles.eyebrow}>
                <T w={600} size={13} color={C.brand}>{page.eyebrow}</T>
              </View>
              <T w={800} size={29} tight accessibilityRole="header">{page.title}</T>
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + 16 }]}>
        <View style={styles.dots}>
          {pages.map((page, i) => (
            <Dot key={page.eyebrow} i={i} x={x} width={width} />
          ))}
        </View>
        <Pressable
          onPress={next}
          ...
          <Animated.View key={last ? 'go' : 'next'} entering={FadeIn.duration(220)} style={styles.ctaInner}>
            <T w={700} size={17} color="#fff">{last ? 'Empezar a ahorrar' : 'Siguiente'}</T>
